...
async function loadEnrichedDatabase() {
    ...
    try {
        // Load main database
        const mainData = await loadDatabaseFile('enriched_database.xlsx', 'main');
        
        // Load top50 database (optional)
        const top50Data = await loadDatabaseFile('top50.xlsx', 'top50', true);

        // Merge databases
        const allData = [...mainData, ...top50Data];

        cachedDatabase = allData;
        lastLoadTime = Date.now();

        const withSpotify = allData.filter(t => t.has_spotify_data).length;
        const top50Count = top50Data.length;

        console.log(`✅ Loaded ${mainData.length} tracks from main database`);
        if (top50Count > 0) {
            console.log(`🎵 Loaded ${top50Count} tracks from Top 50 database`);
        }
        console.log(`📊 Total: ${allData.length} tracks with ${withSpotify} enriched (${Math.round(withSpotify / allData.length * 100)}%)`);
...
// Helper function to load individual database files
async function loadDatabaseFile(filename, type, optional = false) {
    const dbPath = path.join(process.cwd(), 'public', 'data', 'output', filename);
    
    // Check if file exists
    if (!fs.existsSync(dbPath)) {
        if (optional) {
            console.log(`⚠️  Optional ${type} database not found: ${filename}`);
            return [];
        }
        
        // For main database, try input folder as fallback
        if (type === 'main') {
            ...
            return loadDatabaseFile('sortyourmusic.xlsx', 'main_fallback');
        }
        
        throw new Error(`Required database not found: ${filename}`);
    }

    const isEnriched = type !== 'main_fallback';
    const sourcePrefix = type === 'top50' ? 'Top 50 - Nederland' : 'sortyourmusic database';

    console.log(`📊 Loading ${type} database from: ${path.relative(process.cwd(), dbPath)}`);
    ...
    const processedData = rawData.map((track, index) => {
        // Normaliseer kolomnamen naar lowercase
        const normalizedTrack = {};
        Object.keys(track).forEach(key => {
            normalizedTrack[key.toLowerCase()] = track[key];
        });
        
        // Gebruik genormaliseerde track voor verdere verwerking
        const baseTrack = {
            // Basis track info
            title: (normalizedTrack.title || '').toString().trim(),
            artist: (normalizedTrack.artist || '').toString().trim(),
            release: normalizedTrack.release || '',
            length: normalizedTrack.length || '',
            source: normalizedTrack.source || sourcePrefix,
        

            // Voeg language toe uit de genormaliseerde data
            ...
            source_database: type === 'top50' ? 'top50_hits' : (isEnriched ? 'enriched_sortyourmusic' : 'original_sortyourmusic'),
            duplicate_check: track['Duplicate Check'] || '',
            is_current_hit: type === 'top50' // NEW: Flag for current hits
        };
    ...
    if (type === 'main' || type === 'main_fallback') {
