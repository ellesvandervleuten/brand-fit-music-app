...
export function getGenresFromQuestionnaire(brandData, culturalContext = null) {
    ...
    const genreHints = getVibeSpecificGenreHints(brandData.q6, brandData.q2);
    ...
    if (brandData.q7 && brandData.q7.length > 0) {
        console.log('Applying exclusions to secondary genres:', brandData.q7);
        ...
        brandData.q7.forEach(excludedId => {
...
// ---------- Feature profile ----------
// UPDATED calculateOptimalMusicProfile with RADICALIZED differentiation
export function calculateOptimalMusicProfile({ brandData, websiteAnalysis, menuAnalysis }) {
    const {
        ...
        q5: timeBasedAtmosphere,    // Tijd-gebaseerde sfeer
        q6: vibes,                  // Vibes woorden
        q7: exclusions              // Exclusions
    } = brandData || {};

    let reasoning = [];

    // STEP 1: BEDRIJFSDOEL ALS BASIS (40% weight)
    const operationalProfile = operationalGoals[operationalGoal] || operationalGoals.balanced_operation;
    ...
    // LAYER 4: Time-Based Atmosphere (8% weight) - q5
    ...
    // LAYER 5: RADICALIZED Vibes Woorden (5% weight - but much bigger adjustments!)
    if (vibes && vibes.length) {
        ...
        vibes.forEach((vibeWord) => {
            ...
            Object.keys(vibeAdj).forEach((feature) => {
                adj[feature] = (adj[feature] || 0) + (vibeAdj[feature] || 0); // Removed 0.4 multiplier - adjustments are already bigger now
            });
        });
        ...
        reasoning.push(`✨ RADICAL Vibe Words (5%): ${vibes.join(', ')} - Enhanced differentiation`);
    ...
    // Set year preferences based on detected characteristics
    if (isModernOrYouthful && !isTraditionalOrClassic) {
        year_preferences = {
            min_year: 2010,
            preferred_years: [2018, 2024],
            recency_weight: 0.45,
            boost_contemporary_genres: true,
            description: 'Modern/Youthful - Recent music preference'
        };

        // Add contemporary genres
        ...
        reasoning.push('📅 Year Preference: Modern/Recent music heavily favored (2015+)');

    } else if (isTraditionalOrClassic && !isModernOrYouthful) {
        year_preferences = {
            min_year: 1940,
            preferred_years: [1960, 2010],
            recency_weight: -0.6,
            boost_contemporary_genres: false,
            description: 'Traditional/Classic - Older music accepted'
        };

        reasoning.push('📅 Year Preference: Classic/Traditional music accepted (1960+)');

    } else if (isModernOrYouthful && isTraditionalOrClassic) {
        year_preferences = {
            min_year: 1990,
            preferred_years: [2000, 2024],
            recency_weight: 0.2,
            boost_contemporary_genres: false,
            description: 'Mixed signals - Balanced approach'
        };

        reasoning.push('📅 Year Preference: Balanced modern/classic approach (1990+)');
    }
    ...
    if (culturalContext && culturalContext.culture) {
        if (year_preferences) {
            ...
            year_preferences = {
                ...
                recency_weight: 0.1,
                boost_contemporary_genres: false,
                description: `${culturalContext.culture} cultural context`
            };
            reasoning.push(`🎭 Cultural year preference: ${culturalContext.culture} context (1980+)`);
        }
    }
    ...
    return {
        ...
        weight_distribution: {
            ...
            vibe_atmosphere: '12%',
            time_based: '8%',
            vibe_words: '5% (RADICALIZED)',
            note: 'Vibe adjustments increased 3-5x for better differentiation'
        }
    };
}
